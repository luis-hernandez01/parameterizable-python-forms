...
# servicio para listar  los registros
    def list_departamento(self, skip: int, limit: int):
        return self.db.query(Departamento).filter(Departamento.activo == True).offset(skip).limit(limit).all()
    def count_departamento(self):
        return self.db.query(Departamento).filter(Departamento.activo == True).count()
