...
from src.schemas.unidad_ejecutora_schema import (UnidadEjecutoraListResponse, 
...
async def create_unidades(request: Request, 
    ...
    for db in dbs:
        result = await UnidadEjecutoraService(db).create_unidad(payload, request, tokenpayload)
        data.append(result)
...
# crear registrro con uan BD y esta dependencia se agregaria asi => db: Session = Depends(lambda: next(get_session(0)))
    ...
    for db in dbs:
        result = await UnidadEjecutoraService(db).update_unidad(unidad_id, payload, request, tokenpayload)
        data.append(result)
...
async def delete_unidades(request: Request, 
                        unidad_id: int, 
                        # de esta manera llamo todas las bases de datos existentes
                        dbs: list[Session] = Depends(lambda: next(get_session())),
                        tokenpayload: dict = Depends(verify_jwt_token)):
    ...
    data = []
    for db in dbs:
        result = await UnidadEjecutoraService(db).delete_unidad(unidad_id, request, tokenpayload)
        data.append(result)
