...
async def creates(request: Request, 
                        ...
                        tokenpayload: dict = Depends(verify_jwt_token)):
    result = await PresentaService(dbs).create(payload, request, tokenpayload)
    return {"data": result}
...
async def update(request: Request, 
                        ...
                        dbs: list[Session] = Depends(lambda: next(get_session())),
                        tokenpayload: dict = Depends(verify_jwt_token)):
    result = await PresentaService(dbs).updates(presenta_id, payload, request, tokenpayload)
    return {"data": result}
...
async def delete(request: Request, 
                        ...
                        dbs: list[Session] = Depends(lambda: next(get_session())),
                        tokenpayload: dict = Depends(verify_jwt_token)):
    result = await PresentaService(dbs).deletes(presenta_id, request, tokenpayload)
    return {"data": result}
...
async def reactivates(request: Request, 
                        ...
                        dbs: list[Session] = Depends(lambda: next(get_session())),
                        tokenpayload: dict = Depends(verify_jwt_token)):
    result = await PresentaService(dbs).reactivate(presenta_id, request, tokenpayload)
    return {"data": result}